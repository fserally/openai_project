...
class DQNRobotSolver():
    ...
    def run(self, num_episodes, do_train=False):
        ...
        for e in range(num_episodes):
           ...
           #     print('[Episode {}] - Mean survival time over last {} episodes was {} ticks.'.format(e, min_episodes ,mean_score))
            ...
            h, m = divmod(m, 60)
            print('Episode: {}'.format(e+1) + " Time: %d:%02d:%02d" % (h, m, s) +  "Reward: " + str(cumulated_reward))
    ...
    def load(self, model_name, models_dir_path="/tmp"):
        ...
        json_file = open(model_name_json_format_path, 'r')
...
if __name__ == '__main__':
    ...
    rospy.init_node('turtlebot3_deep_qlearn', anonymous=True, log_level=rospy.DEBUG)
    ...
    model_name = "turtlebot3_"

    environment_name = 'TurtleBot3World-v0'

    n_observations = rospy.get_param('/turtlebot3/n_observations')

    print("observations" + str(n_observations))
    n_actions = rospy.get_param('/turtlebot3/n_actions')

    n_episodes_training = rospy.get_param('/turtlebot3/episodes_training')
    n_episodes_running = rospy.get_param('/turtlebot3/episodes_running')
    n_win_ticks = rospy.get_param('/turtlebot3/n_win_ticks')
    min_episodes = rospy.get_param('/turtlebot3/min_episodes')
    max_env_steps = None
    gamma =  rospy.get_param('/turtlebot3/gamma')
    epsilon = rospy.get_param('/turtlebot3/epsilon')
    epsilon_min = rospy.get_param('/turtlebot3/epsilon_min')
    epsilon_log_decay = rospy.get_param('/turtlebot3/epsilon_decay')
    alpha = rospy.get_param('/turtlebot3/alpha')
    alpha_decay = rospy.get_param('/turtlebot3/alpha_decay')
    batch_size = rospy.get_param('/turtlebot3/batch_size')
    monitor = rospy.get_param('/turtlebot3/monitor')
    quiet = rospy.get_param('/turtlebot3/quiet')
